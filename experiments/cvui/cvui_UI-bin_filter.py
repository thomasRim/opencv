...
recog_btn_states = {0: "Start recognition",
            1: "Stop recognition"}
recog_btn_state = [recog_btn_states.get(0, "None")]

# recognizers state
recog_states = {0: "Contours >",
            1: "< YOLO"}
recog_state = [recog_states.get(0, "None")]

# contour debug state
contour_states = {0: "normal",
            1: "thresholded"}
...
(stop_all_b_x,
  stop_all_b_y,
  stop_all_b_w,
  stop_all_b_h) = (int(0.016 * win_W), int(0.83 * win_H), int(0.274 * win_W), int(0.16 * win_H))
(reset_crop_b_x,
  reset_crop_b_y,
  reset_crop_b_w,
  reset_crop_b_h) = (int(0.016 * win_W), int(0.586 * win_H), int(0.06 * win_W), int(0.06 * win_H))
...
(recog_set_x,
  recog_set_y,
  recog_set_w,
  recog_set_h) = (int(0.3 * win_W), int(0.66 * win_H), int(0.33 * win_W), int(0.33 * win_H))

# recognizers setup toggle button - yolo/contour
(recog_set_btn_x,
  recog_set_btn_y,
  recog_set_btn_w,
  recog_set_btn_h) = (int(0.305 * win_W), int(0.665 * win_H), int(0.32 * win_W), int(0.03 * win_H))

# recognizers trackbars
## threshold min
(trecog_thr_min_x, trecog_thr_min_y, trecog_thr_min_w) = (int(0.42 * win_W), int(0.7 * win_H), int(0.195 * win_W))
## threshold max
(trecog_thr_max_x, trecog_thr_max_y, trecog_thr_max_w) = (int(0.42 * win_W), int(0.75 * win_H), int(0.195 * win_W))
## alpha
(trecog_alpha_x, trecog_alpha_y, trecog_alpha_w) = (int(0.42 * win_W), int(0.8 * win_H), int(0.195 * win_W))
## beta
(trecog_beta_x, trecog_beta_y, trecog_beta_w) = (int(0.42 * win_W), int(0.85 * win_H), int(0.195 * win_W))
## contour area min
(trecog_area_min_x, trecog_area_min_y, trecog_area_min_w) = (int(0.42 * win_W), int(0.9 * win_H), int(0.195 * win_W))
## contour area max
(trecog_area_max_x, trecog_area_max_y, trecog_area_max_w) = (int(0.42 * win_W), int(0.95 * win_H), int(0.195 * win_W))
...
(recog_res_x,
  recog_res_y,
  recog_res_w,
  recog_res_h) = (int(0.64 * win_W), int(0.66 * win_H), int(0.35 * win_W), int(0.33 * win_H))
...
def detectContoursFrom(img, frame, alpha, beta, gamma, area_min, area_max):
    ...
    _, threshold_img = cv2.threshold(gray, threshold_min[0], threshold_max[0], cv2.THRESH_BINARY)
    ...
    for cnt in good_contours:
        ...
        cv2.putText(img, "{:}, {}".format(obj_index, str(area)), (x, y), 1, 1, (0, 255, 0))
...
def cropTrackBars(frame, cv_sh_w, cv_sh_h):
    # reset crop button
    if cvui.button(frame, reset_crop_b_x, reset_crop_b_y, reset_crop_b_w, reset_crop_b_h, "Reset"):
        resetCrop()
    #x
...
def contourTrackBars(frame):
    #threshold
    cvui.text(frame, trecog_thr_min_x - int(0.1 * win_W), trecog_thr_min_y + int(0.015 * win_H), "Threshold min")
    cvui.trackbar(frame, trecog_thr_min_x, trecog_thr_min_y, trecog_thr_min_w, threshold_min, 1, threshold_max[0] - 1)
    
    cvui.text(frame, trecog_thr_max_x - int(0.1 * win_W), trecog_thr_max_y + int(0.01 * win_H), "Threshold max")
    cvui.trackbar(frame, trecog_thr_max_x, trecog_thr_max_y, trecog_thr_max_w, threshold_max, threshold_min[0], 255)
    
    # image correction
    cvui.text(frame, trecog_alpha_x - int(0.1 * win_W), trecog_alpha_y + int(0.015 * win_H), "Correct Alpha")
    cvui.trackbar(frame, trecog_alpha_x, trecog_alpha_y, trecog_alpha_w, img_alpha, 1.0, 3.0, 0.1)

    cvui.text(frame, trecog_beta_x - int(0.1 * win_W), trecog_beta_y + int(0.015 * win_H), "Correct Beta")
    cvui.trackbar(frame, trecog_beta_x, trecog_beta_y, trecog_beta_w, img_beta, 1, 100)

    # area
    cvui.text(frame, trecog_area_min_x - int(0.1 * win_W), trecog_area_min_y + int(0.015 * win_H), "Area min")
    cvui.trackbar(frame, trecog_area_min_x, trecog_area_min_y, trecog_area_min_w, contour_area_min, 1.0, contour_area_max[0]-1)

    cvui.text(frame, trecog_area_max_x - int(0.1 * win_W), trecog_area_max_y + int(0.015 * win_H), "Area max")
    cvui.trackbar(frame, trecog_area_max_x, trecog_area_max_y, trecog_area_max_w, contour_area_max, contour_area_min[0], 20000)
    ...
    while True:
        ...
        if cap.isOpened():
            ...
            if ret:
                ...
                if cvui.button(frame, recog_set_btn_x, recog_set_btn_y, recog_set_btn_w, recog_set_btn_h, recog_state[0]):
                    stopRecognize()
                    toggleRecognizers()
                ...
                if recognize[0] == True:
                    if recog_state[0] == recog_states.get(0, ""):
                        detectContoursFrom(crop_image, frame, img_alpha[0], img_beta[0], img_gamma[0], contour_area_min[0], contour_area_max[0])
        ...
        if cvui.button(frame, recog_b_x, recog_b_y, recog_b_w, recog_b_h, recog_btn_state[0]):
            if recognize[0] == True:
                stopRecognize()
            else:
                startRecognize()
